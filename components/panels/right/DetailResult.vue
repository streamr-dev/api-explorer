<template lang="pug">
  v-expansion-panel(v-if="item._._result" expand :class="VIEW_DARK ? 'theme--dark' : 'theme--light'")
    v-expansion-panel-content(v-model="exp1", ripple, v-if="item._._error")
      ...
      v-divider
      v-alert.ma-0(error icon="warning" value="true") {{item._._error.message || item._._error}}. Ensure you have authenticated with Metamask for permissioned endpoints (Lock icon in the header).
